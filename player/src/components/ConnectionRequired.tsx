...
const ConnectionRequired: FunctionComponent<ConnectionRequiredProps> = ({
    ...
    const handleWizard = useCallback(async (firstConnection: ConnectResponse | null) => {
        ...
        // Umbry: Emby's public info omits StartupWizardCompleted; treat only an explicit false
        // as "wizard needed" so connecting to an Emby server never bounces into the setup wizard.
        if (systemInfo?.StartupWizardCompleted !== false) {
    ...
    useEffect(() => {
        // Umbry: if a Plex server was the active session, restore its shim before we
        // decide where to route, so a refresh doesn't bounce out to the server-select.
        (window as unknown as { __jpxRestorePlex?: () => void }).__jpxRestorePlex?.();
