...
export function getCardImageUrl({
    ...
    // Umbry: Plex items carry ready-made image URLs (Plex uses a different image API than
    // Jellyfin), so short-circuit the SDK URL builder and use them directly.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const plexItem = item as any;
    if (plexItem && typeof plexItem.ServerId === 'string' && plexItem.ServerId.indexOf('plex_') === 0) {
        const wantThumb = options.preferThumb || shape === CardShape.Backdrop;
        const purl = (wantThumb && plexItem.__thumbImageUrl)
            ? plexItem.__thumbImageUrl
            : (plexItem.__primaryImageUrl || plexItem.__thumbImageUrl || plexItem.__backdropUrl);
        return { imgUrl: purl || undefined, blurhash: undefined, forceName: false, coverImage: false };
    }
