...
function createStreamInfoFromUrlItem(item) {
    ...
    return {
        ...
        // Umbry: books/comics take this URL-item path and skip PlaybackInfo, so they never get a
        // PlaySessionId. Jellyfin tolerates a null session on progress reports (it just upserts the
        // resume position by item), but EMBY rejects it with 400 "Value cannot be null (key)",
        // so reading progress never saved on Emby → no Continue Reading. Generate a client
        // PlaySessionId so playback-start registers a session and progress reports match it.
        playSessionId: (Date.now().toString(16) + Math.random().toString(16).slice(2) + Math.random().toString(16).slice(2)).replace(/[^a-f0-9]/g, '').slice(0, 32).padEnd(32, '0')
    };
}
...
function getAudioStreamUrl(item, transcodingProfile, directPlayContainers, apiClient, startPosition, maxValues) {
    ...
    return apiClient.getUrl(url, {
        ...
        // Umbry: 'api_key' (not 'ApiKey') — Emby rejects the newer Jellyfin param name with a 401,
        // which broke ALL music on Emby servers. Jellyfin accepts both spellings.
        ...
        function playAfterBitrateDetect(maxBitrate, item, playOptions, onPlaybackStartedFn, prevSource) {
            ...
            if (!isServerItem(item) || item.MediaType === 'Book') {
                return promise.then(function () {
                    ...
                    // Umbry: store the streamInfo so playSessionId(player) resolves — without it the
                    // book's generated PlaySessionId never reaches the report payload and Emby 400s.
            ...
            // Umbry: Emby MediaSource ids are 'mediasource_<id>' (not fetchable item ids), so
            // using one as an item id 500s and kills playback before PlaybackInfo. Only fetch by the
            // media-source id when it looks like a real item (Jellyfin alternate versions); otherwise
            // use item.Id. A catch retries with item.Id as a final backstop.
            const streamsItemId = (mediaSourceId && String(mediaSourceId).indexOf('mediasource_') !== 0) ? mediaSourceId : item.Id;
            const getMediaStreams = isLiveTv ? Promise.resolve([]) : apiClient.getItem(apiClient.getCurrentUserId(), streamsItemId)
                .catch(() => apiClient.getItem(apiClient.getCurrentUserId(), item.Id))
                .then(fullItem => {
            ...
            return Promise.all([promise, player.getDeviceProfile(item), apiClient.getCurrentUser(), getMediaStreams]).then(function (responses) {
                ...
                return getPlaybackMediaSource(player, apiClient, deviceProfile, item, mediaSourceId, options).then(async (mediaSource) => {
                    ...
                    return player.play(streamInfo).then(function () {
                        ...
                        onPlaybackStartedFn();
                        onPlaybackStarted(player, playOptions, streamInfo, mediaSource);
                        // Umbry diagnostic: surface the REAL play failure (the dialog collapses many
                        // causes into "fatal player error"). Grab this from DevTools console.
                        try { console.error('[UMBRY PLAYBACK ERROR]', 'name=' + (err && err.name), 'msg=' + (err && err.message), 'raw=' + String(err), 'type=' + (((err != null) && Object.values(MediaError).includes(err)) ? err : getMediaError(err)), 'playMethod=' + (streamInfo && streamInfo.playMethod), 'proto=' + (streamInfo && streamInfo.mediaSource && streamInfo.mediaSource.Protocol), 'url=' + String((streamInfo && streamInfo.url) || '').slice(0, 180)); } catch (e) { /* ignore */ }
                        setTimeout(function () {
                            onPlaybackError.call(player, err, {
                                type: ((err != null) && Object.values(MediaError).includes(err)) ? err : getMediaError(err),
                                streamInfo
                            });
                        }, 100);
                    });
            ...
            if (type === 'Video' || type === 'Audio') {
                ...
                if (mediaSource.enableDirectPlay) {
                    ...
                    directOptions = {
                        ...
                        deviceId: apiClient.deviceId(),
                        api_key: apiClient.accessToken() // Umbry: Emby rejects 'ApiKey' (401); both servers accept 'api_key'
                    };
