...
class AppRouter {
    ...
    getRouteUrl(item, options) {
        ...
        // Content folders (Type 'Folder' — e.g. book series / per-book download folders) route to
        // our custom detail page, which renders them as a scoped grid, instead of the stock #/list.
        if (itemType === 'Folder' && id) {
            return '#/details?id=' + id + '&serverId=' + serverId;
        }
