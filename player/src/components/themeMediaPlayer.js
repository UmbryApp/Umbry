...
function playThemeMedia(items, ownerId) {
    ...
    if (currentThemeItems.length) {
        ...
        playbackManager.play({
            ...
            // Umbry: apply Theme Music volume + loop prefs to the player.
            try {
                const vol = Number(getPref('themeMusicVolume', 50));
                if (Number.isFinite(vol)) playbackManager.setVolume(Math.max(0, Math.min(100, vol)));
                playbackManager.setRepeatMode(getPref('themeMusicLoop', true) ? 'RepeatOne' : 'RepeatNone');
...
document.addEventListener('viewshow', e => {
    ...
    if (serverId && id) {
        // Umbry: Plex servers have no Jellyfin theme-media endpoint — skip (avoids a console error).
        if (String(serverId).indexOf('plex_') === 0) { playThemeMedia([], null); return; }
