...
build: ${__JF_BUILD_VERSION__}`);
    ...
    // Umbry: when a saved Plex/Jellyfin session is being restored on (re)load, skip the
    // origin auto-connect (it would override the restored session, esp. cross-origin dev).
    let jpxHasSaved = false;
    try { jpxHasSaved = !!(localStorage.getItem('jpx-active-jf') || localStorage.getItem('jpx-active-plex')); } catch (e) { /* ignore */ }
    // Umbry is a standalone client, NOT served by a Jellyfin server, so the app's own origin is
    // not a real server. Auto-connecting to it registers a phantom "self" server (named "Player",
    // pointing at the app URL, tokenless) that clutters the picker. Never auto-connect to origin;
    // real servers are added through the custom picker / restored from a saved session.
    let jpxIsOwnOrigin = false;
    try { jpxIsOwnOrigin = !!serverUrl && String(serverUrl).replace(/\/+$/, '') === String(window.location.origin).replace(/\/+$/, ''); } catch (e) { /* ignore */ }
    if (serverUrl && !jpxHasSaved && !jpxIsOwnOrigin) {
