        ...
        return new Promise((resolve, reject) => {
            requireHlsPlayer(async () => {
                ...
                const hls = new Hls({
                    ...
                    // Umbry: prefer SDR/H.264 variants. Preferring HDR selects HEVC/H.265
                    // playlists that Chrome cannot decode via hls.js/MSE -> audio plays but no video.
                    videoPreference: { preferHDR: false },
                    xhrSetup(xhr, url) {
                        xhr.withCredentials = includeCorsCredentials;
                        // Umbry: Plex transcode playlists/segments are served relative and
                        // token-less; append the active Plex token so these requests authenticate.
                        try {
                            const jpx = window.__jpxPlex;
                            if (jpx && jpx.token && typeof url === 'string' && url.indexOf(jpx.host) === 0 && url.indexOf('X-Plex-Token') === -1) {
                                const u = url + (url.indexOf('?') >= 0 ? '&' : '?') + 'X-Plex-Token=' + encodeURIComponent(jpx.token);
                                xhr.open('GET', u, true);
                            }
                        } catch (e) { /* ignore */ }
                    }
    ...
    createMediaElement(options) {
        ...
        if (!dlg) {
            ...
            if (options.fullscreen) {
                ...
                // Umbry: this container is REUSED across playbacks, and -onTop (its z-index) was removed
                // when the previous playback stopped — re-raise it here, or the reused video plays behind
                // a stale backdrop/layer left over from the view we navigated in from.
            ...
            // Umbry: always refresh the poster (clearing a stale one when there's no new backdrop) so
            // the previous item's backdrop never lingers on top of the newly-playing reused video.
            videoElement.poster = options.backdropUrl || '';
