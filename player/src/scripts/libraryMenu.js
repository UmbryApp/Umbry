...
const fetchServerName = (_apiClient) => {
    _apiClient
        ?.getPublicSystemInfo()
        .then(({ ServerName }) => {
            // Only the CURRENT server drives the tab title. apiclientcreated fires for every
            // credentialed server (e.g. a Jellyfin client created while a Plex session is active),
            // and without this check the last-created server's name would clobber the current one.
            try { if (ServerConnections.currentApiClient() && _apiClient !== ServerConnections.currentApiClient()) return; } catch (e) { /* ignore */ }
...
function setDocumentTitle (name) {
    // Umbry: set the tab's brand/home title. Falls back to the Umbry brand.
    documentTitle = name || 'Umbry';
    document.title = documentTitle;
}
...
Events.on(ServerConnections, 'localusersignedin', function (e, user) {
    ...
    loadNavDrawer();
    fetchServerName(currentApiClient);
