...
export function logout() {
    ServerConnections.logout().then(function () {
        ...
        // Reset cached views
        viewContainer.reset();
        // Umbry: always land on our themed login page — even in multiserver mode, where
        // stock routes to the unstyled `selectserver`. The login view's `viewshow` calls
        // jpxShowServerSelect(), so this shows our custom select/add-a-server screen.
        navigate('login');
    });
}
